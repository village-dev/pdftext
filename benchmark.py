    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for i in range(len(pdf.pages)):
            ...
            lines = page.extract_text_lines(
                strip=False, return_chars=True, keep_text_flow=True
            )
            text = ""
            for line in lines:
...
def main():
    ...
    parser.add_argument(
        "--pdftext_only",
        action="store_true",
        help="Only run pdftext inference",
        default=False,
    )
    parser.add_argument(
        "--pdftext_workers",
        type=int,
        help="Number of workers to use for pdftext inference",
        default=None,
    )
    ...
    for i in tqdm(range(len(dataset)), desc="Benchmarking"):
        ...
        with tempfile.NamedTemporaryFile(suffix=".pdf") as f:
            ...
            pdftext_inference_model = partial(
                pdftext_inference, workers=args.pdftext_workers
            )
            inference_funcs = [
                pymupdf_inference,
                pdftext_inference_model,
                pdfplumber_inference,
            ]
            ...
            for tool in alignment_tools:
                alignments[tool].append(
                    mean(
                        [
                            compare_docs(tool_pages["pymupdf"][i], tool_pages[tool][i])
                            for i in range(len(tool_pages["pymupdf"]))
                        ]
                    )
                )
    ...
    headers = [
        "Library",
        "Time (s per page)",
        "Alignment Score (% accuracy vs pymupdf)",
    ]
    ...
    table = [
        (tool, time, alignment)
        for tool, time, alignment in zip(times_tools, table_times, table_alignments)
    ]
    table = tabulate.tabulate(table, tablefmt="github", headers=headers)
    print(table)

    results = {"times": times, "alignments": alignments}

...
from pdftext.pdf.utils import (
    get_fontname,
    page_bbox_to_device_bbox,
    pdfium_page_bbox_to_device_bbox,
)
from pdftext.settings import settings


def update_previous_fonts(
    char_infos: List,
    i: int,
    prev_fontname: str,
    prev_fontflags: int,
...
def get_pdfium_chars(
    pdf, page_range, flatten_pdf, fontname_sample_freq=settings.FONTNAME_SAMPLE_FREQ
):
    blocks = []
    ...
    for page_idx in page_range:
        ...
        bbox = pdfium_page_bbox_to_device_bbox(
            page, bbox, page_width, page_height, page_rotation
        )
        ...
        for i in range(total_chars):
            ...
            if fontname is None or i % fontname_sample_freq == 0:
                ...
                if (fontname != prev_fontname or fontflags != prev_fontflags) and i > 0:
                    update_previous_fonts(
                        char_infos,
                        i,
                        prev_fontname,
                        prev_fontflags,
                        text_page,
                        fontname_sample_freq,
                    )

            rotation = pdfium_c.FPDFText_GetCharAngle(text_page, i)
            rotation = rotation * rad_to_deg  # convert from radians to degrees
            coords = text_page.get_charbox(
                i, loose=rotation == 0
            )  # Loose doesn't work properly when charbox is rotated
            device_coords = page_bbox_to_device_bbox(
                page, coords, page_width, page_height, page_rotation, normalize=True
            )

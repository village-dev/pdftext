...
def get_fontname(textpage, char_index):
    ...
    font_info = pdfium_c.FPDFText_GetFontInfo(
        textpage, char_index, buffer_ptr, n_bytes, flag_buffer
    )
    if font_info == 0:
        return None, None
    try:
...
def page_to_device(
    page, x, y, page_width, page_height, page_rotation: int, device_x, device_y
):
    if page_rotation == 90:
        page_rotation = 1
    ...
    pdfium_c.FPDF_PageToDevice(
        page, 0, 0, page_width, page_height, page_rotation, x, y, device_x, device_y
    )
...
def pdfium_page_bbox_to_device_bbox(page, bbox, page_width, page_height, page_rotation):
    ...
    left_bottom = page_to_device(
        page, *bbox[:2], page_width, page_height, page_rotation, device_x, device_y
    )
    top_right = page_to_device(
        page, *bbox[2:], page_width, page_height, page_rotation, device_x, device_y
    )
...
def page_bbox_to_device_bbox(
    page, bbox, page_width: int, page_height: int, page_rotation: int, normalize=False
):
    ...
    bbox = pdfium_page_bbox_to_device_bbox(
        page, bbox, orig_page_width, orig_page_height, page_rotation
    )
    if page_rotation > 0:
